...
class Cmauth {
  ...
  /**
   * function to check if the current user is logged in by check their session
   * variable
   *
   * @return boolean
   */
  ...
  public function log_user_in($username = NULL, $password = NULL) {
    ...
    $query = $this->_CI->db->where('username', $username)
                           ->where('active', 1)
                           ->limit(1)
                           ->get('users');
  ...
  public function create_lostpassword_email($email = NULL) {
    ...
    $query = $this->_CI->db->where('email', $email)
                           ->where('active', 1)
                           ->limit(1)
                           ->get('users');
    $error = $this->_CI->db->error();
  ...
  public function check_reset_hash($hash = NULL) {
    ...
    $query = $this->_CI->db->where('password_token', $hash)
                           ->where('active', 1)
                           ->get('users');
    $error = $this->_CI->db->error();
